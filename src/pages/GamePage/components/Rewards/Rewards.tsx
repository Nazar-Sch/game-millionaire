    ...
    <div className='rewards__container'>
      {REWARDS.sort((a, b) => b - a).map((reward, index) => (
        <div className='rewards-wrapper' key={index}>
          <div className={cx('rewards-item__left-line', getAditionalClassName(reward, currentReward))}></div>
          <div className={cx('rewards-item__item', getAditionalClassName(reward, currentReward))}>{convertToUSD(reward)}</div>
          <div className={cx('rewards-item__right-line', getAditionalClassName(reward, currentReward))}></div>
        </div>
      ))}
    </div>
  );
